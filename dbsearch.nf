...
process run_comet {
    ...
    output:
    file ('./result.txt') into result_file_ch

    script:
    """
    #!/bin/sh
    mkdir psm
    comet -P${baseDir}/config/comet_params.txt -N./result $spectrum_file 
    """
}
...
process generate_input {
    ...
    input:
    file (spectrum_file)
    file (result_file) from result_file_ch
