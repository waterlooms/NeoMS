...
calculate_similarity <- function(x, output) {
  ...
  pdeep2 <- read.table(text=gsub("(?<=[a-z])\\s+", "\n", x[4], perl=TRUE),header=FALSE, col.names = c("mz", "intensity"))
  ...
  test$intensity.x <- test$intensity.x/sqrt(sum(test$intensity.x^2))
  test$intensity.y <- test$intensity.y/sqrt(sum(test$intensity.y^2))
...
output_data <- title_list_result %>%
        select(index, pdeep_title, similarity)
